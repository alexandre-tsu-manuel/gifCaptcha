<?php
	...
	*/
	Class GIFEncoder
	{
		private $GIF;	/* GIF header 6 bytes	*/
		private $VER;	/* Encoder version		*/
		private $BUF;
		private $LOP;
		private $DIS;
		private $COL;
		...
		public function __construct($GIF_src, $GIF_dly, $GIF_lop, $GIF_dis, $GIF_red, $GIF_grn, $GIF_blu, $GIF_mod)
		{
			...
			for ( $i = 0; $i < count ( $GIF_src ); $i++ )
			{
				if ( strToLower ( $GIF_mod ) == "url" )
					$this->BUF [$i] = fread ( fopen ( $GIF_src [ $i ], "rb" ), filesize ( $GIF_src [ $i ] ) );
				else
					$this->BUF [$i] = $GIF_src [ $i ];
			}
